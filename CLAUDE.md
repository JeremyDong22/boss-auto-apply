...
- [ ] **评价卡片导致提前停止（待确认）**：职位列表中间会插入一张"评价"类卡片（非投递卡片），脚本遇到它后会误判"到底了"停止运行，但该卡片下方仍有更多职位卡片未投递。
  - 现象：脚本在 15 个左右停止，显示"到底了"，但页面还有更多职位
  - 怀疑原因 A：评价卡片点击后触发了未知弹窗，脚本不识别无法关闭，后续所有卡片都 skipped，遍历完 15 张后触发 tryScrollForMore，但 Boss 的懒加载依赖该弹窗被关闭才触发，导致 4 次滚动都没有新卡片 → 误判到底
  - 怀疑原因 B：评价卡片本身不是 `.job-card-wrap`，但它的存在阻断了 Boss 的懒加载触发机制（IntersectionObserver 哨兵在它后面）
  - v13.7 临时缓解：tryScrollForMore 等待时间 500ms→1000ms，减少加载慢时的误判
  - **待办**：用 `bookmarklet_grab_page_html.js` 在评价卡片出现时抓取 HTML，确认：
    1. 评价卡片的 class（是否含 `.job-card-wrap`）
    2. 点击后是否触发弹窗（弹窗 class 是什么）
    3. 确认后针对性处理（检测到评价卡片 → 跳过不点击，或关闭弹窗）
